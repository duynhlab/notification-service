...
func LoggingMiddleware(logger *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Prefer the active span's trace ID so logs correlate with exported
		// traces; fall back to header/generated ID when no span is present.
		...
		// Single request log per request; escalate to error level for 4xx/5xx
		// so failures stand out without emitting a second, duplicate entry.
		fields := []zap.Field{
