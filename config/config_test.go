...
func TestBuildDSN(t *testing.T) {
	db := &DatabaseConfig{
		Host: "localhost", Port: "5432", Name: "notification",
		User: "notification", Password: "secret", SSLMode: "disable",
	}
	want := "postgresql://notification:secret@localhost:5432/notification?sslmode=disable"
...
func TestLoad_Overrides(t *testing.T) {
	t.Setenv("SERVICE_NAME", "notification")
	t.Setenv("PORT", "9999")
...
func validConfig() *Config {
	c := &Config{}
	c.Service = ServiceConfig{Name: "notification", Port: "8080", Env: "production"}
	c.Tracing = TracingConfig{Enabled: true, Endpoint: "otel:4318", SampleRate: 0.1, ServiceName: "notification"}
	c.Profiling = ProfilingConfig{Enabled: true, Endpoint: "pyro:4040", ServiceName: "notification"}
...
func TestGetEnvHelpers(t *testing.T) {
	...
	t.Run("getEnvFloat falls back on invalid", func(t *testing.T) {
		t.Setenv("X_FLOAT", "notnum")
		if got := getEnvFloat("X_FLOAT", 0.25); got != 0.25 {
			t.Errorf("getEnvFloat(invalid) = %v, want 0.25", got)
		}
		t.Setenv("X_FLOAT", "0.75")
		if got := getEnvFloat("X_FLOAT", 0.25); got != 0.75 {
			t.Errorf("getEnvFloat(0.75) = %v, want 0.75", got)
		}
	})
}
